...
function M.fill_with_pattern(pattern)
  ...
  -- break column.
  local num_rep = math.floor((textwidth - subline_dlen)/pattern_len) - 1
  ...
  if num_rep >= 0 then
    -- Fill the line with the character `char`.
    local char_id = utf8_aux.get_char_index_at_bytepos(line[1], cur_pos[2])
